...
## Folder structure
**The migration process will NOT be a straight copy from LS2 to CSENS**

We have three file stores on LS2 where you keep your data:
...
`/raw` is backed up, `/processed` is **not**. This means all your raw data (fastq etc) should be kept in `/raw`, and all of your intermediate files should be sent to `/processed`. Your `/home` is also backed up, so your scripts should be kept here. This idea is that if you lose an intermediate file, you should be able to regenerate it from your raw data with your scripts in `/home`.
...
## Initial prep
1) LUNARC will take your whitlisted IP's from LS2, so nothing needs to be done there.

2) Make sure you can still access LSENS2

3) It make sense that you do a cleanup of your files on LSENS2 before you copy over.  
...
The only way to use COSMOS-SENS is via a web browser which you should point to:
...
If you are asked to change you password, do not do it! Instead, change your password at:
...
1) **Login to COSMOS-SENS** and make a folder. For example `mkdir /processed/<userid>/FileTransferFolder`
...
`sshfs <userID>@cs-diode:/processed/<userid>/FileTransferFolder  /home/<userid>/ToCSENS`

4) **On LS2** you can now copy files into `ToCSENS` where they will be available in COSMOS-SENS.

5) When you are done copying to the mounted folder, you can unmount the folder by doing on **LSENS**
...
6) Now that you have your files on COSMOS-SENS you should arrange itso your raw data goes to `/raw` and the rest on `/processed/`, and your srcipts to `/home`.

**From this point onwards you should only work on COSMOS-SENS**.

## Missing software

COSMOS-SENS uses a different CPU architecture to LSENS, so some software will be mssing. If you need something installed please contact Shamit so a request can be put in.